...
* **Account Status Messaging:** Added explicit login error messages for Suspended, Unofficially Dropped (UD), and Dropped accounts, while maintaining the standard pending approval message for Inactive accounts.
* **Group Members Modal:** Implemented a click-to-view modal for group members. Students can click their group name in the My Course Summary, and lecturers can click group names in the Roster List to instantly view all assigned members.
